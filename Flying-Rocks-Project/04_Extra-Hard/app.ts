...
function winningTime(winnerTime:number):string{
    const minutes = Math.floor(winnerTime/60);
    const seconds = winnerTime % 60;
    const winnnerSec = addingZero(seconds);
    const winnerMin = addingZero(minutes);
